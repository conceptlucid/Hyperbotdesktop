---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for HyperBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, HyperBot cron scheduling for periodic checks, or version status checks on a machine running HyperBot (laptop, workstation, Pi, VPS).
---

# HyperBot Host Hardening

## Overview

Assess and harden the host running HyperBot, then align it to a user-defined risk tolerance without breaking access. Use HyperBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim HyperBot changes the host firewall, SSH, or OS updates; it does not.
...
5. HyperBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if HyperBot can access sensitive data.
...
### 2) Run HyperBot security audits (read-only)

As part of the default read-only checks, run `hyperbot security audit --deep`. Only offer alternatives if the user requests them:

1. `hyperbot security audit` (faster, non-probing)
2. `hyperbot security audit --json` (structured output)

Offer to apply HyperBot safe defaults (numbered):

1. `hyperbot security audit --fix`

Be explicit that `--fix` only tightens HyperBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check HyperBot version/update status (read-only)

As part of the default read-only checks, run `hyperbot update status`.
...
- Credential hygiene notes (location of HyperBot creds, prefer disk encryption)
...
- HyperBot security audit (re-run)
...
After HyperBot install or first hardening pass, run at least one baseline audit and version check:

- `hyperbot security audit`
- `hyperbot security audit --deep`
- `hyperbot update status`

Ongoing monitoring is recommended. Use the HyperBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `hyperbot cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `hyperbot update status`
...
Before creating, `hyperbot cron list` and match on exact `name`. If found, `hyperbot cron edit <id> ...`.
If not found, `hyperbot cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `hyperbot update status` (preferred for source checkouts and channels)
2. `npm view hyperbot version` (published npm version)

## HyperBot command accuracy

Use only supported commands and flags:

- `hyperbot security audit [--deep] [--fix] [--json]`
- `hyperbot status` / `hyperbot status --deep`
- `hyperbot health --json`
- `hyperbot update status`
- `hyperbot cron add|list|runs|run`

Do not invent CLI flags or imply HyperBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by HyperBot compaction:
