---
name: node-connect
description: Diagnose HyperBot node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify HyperBot is advertising that route, then fix pairing/auth.
...
- whether `hyperbot devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `hyperbot qr --json`. It uses the same setup-code payload Android scans.

```bash
hyperbot config get gateway.mode
hyperbot config get gateway.bind
hyperbot config get gateway.tailscale.mode
hyperbot config get gateway.remote.url
hyperbot config get gateway.auth.mode
hyperbot config get gateway.auth.allowTailscale
hyperbot config get plugins.entries.device-pair.config.publicUrl
hyperbot qr --json
hyperbot devices list
hyperbot nodes status
```

If this HyperBot instance is pointed at a remote gateway, also run:

```bash
hyperbot qr --remote --json
```
...
`hyperbot qr --json` success means:
...
If `hyperbot qr --json` says `Gateway is only bound to loopback`:
...
- `hyperbot devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run hyperbot qr again, rescan, then approve the pending device pairing.`
