...
metadata: { "hyperbot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == HyperBot `sessions_spawn`
- **File I/O** == HyperBot `read`/`write`
- **Remote fetch** == HyperBot `web_fetch` (or `exec` with curl when POST is required)
