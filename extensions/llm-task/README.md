...
`hyperbot.invoke --each`) without adding new HyperBot code per workflow.
...
This extension depends on HyperBot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** HyperBot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.hyperbot/extensions` as a standalone plugin directory.
