...
Read-only diff viewer plugin for **HyperBot** agents.
...
Before:
HyperBot supports plugins.

After:
HyperBot supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/hyperbot-diffs`).
