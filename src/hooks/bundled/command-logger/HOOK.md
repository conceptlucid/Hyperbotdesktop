...
homepage: https://docs.ancient-claw.ai/automation/hooks#command-logger
metadata:
  {
    "ancient-claw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.ancient-claw/logs/commands.log`
...
`~/.ancient-claw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.ancient-claw/logs/commands.log ~/.ancient-claw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/ancient-claw`:
   ```
   /home/username/.ancient-claw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
