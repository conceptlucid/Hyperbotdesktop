...
homepage: https://docs.hyperbot.ai/automation/hooks#command-logger
metadata:
  {
    "hyperbot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.hyperbot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.hyperbot/logs/commands.log ~/.hyperbot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/hyperbot`:
   ```
   /home/username/.hyperbot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
