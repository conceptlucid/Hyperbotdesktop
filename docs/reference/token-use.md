---
summary: "How Ancient Claw builds prompt context and reports token usage + costs"
read_when:
...
Ancient Claw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Ancient Claw assembles its own system prompt on every run. It includes:
...
For images, Ancient Claw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Ancient Claw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `ancient-claw status --usage` and `ancient-claw channels list` show
...
`cacheWrite`. If pricing is missing, Ancient Claw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Ancient Claw can
...
Anthropic's 1M context window is currently beta-gated. Ancient Claw can inject the
...
Ancient Claw skips the `context-1m-*` beta header because Anthropic currently
