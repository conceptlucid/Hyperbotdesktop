...
This document explains how Ancient Claw manages sessions end-to-end:
...
Ancient Claw is designed around a single **Gateway process** that owns session state.
...
Ancient Claw persists sessions in two layers:
...
- Transcripts: `~/.ancient-claw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

Ancient Claw resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, Ancient Claw reports potential evictions but does not mutate the store/files.
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.ancient-claw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
Ancient Claw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (Ancient Claw consumes the events, but Pi decides when to compact).
...
Ancient Claw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, Ancient Claw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, Ancient Claw leaves it alone.
...
- `ancient-claw status` (CLI)
- `ancient-claw sessions` / `sessions --json`
...
Ancient Claw supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- Ancient Claw strips/suppresses this in the delivery layer.

As of `2026.1.10`, Ancient Claw also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
Ancient Claw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but Ancient Claw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `ancient-claw status`.
