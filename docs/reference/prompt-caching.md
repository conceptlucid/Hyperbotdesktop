...
- With Anthropic API-key auth profiles, Ancient Claw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Ancient Claw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Ancient Claw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.ancient-claw/logs/cache-trace.jsonl" # optional
