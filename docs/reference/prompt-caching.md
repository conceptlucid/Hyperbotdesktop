...
- With Anthropic API-key auth profiles, HyperBot seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, HyperBot injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
HyperBot exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.hyperbot/logs/cache-trace.jsonl" # optional
