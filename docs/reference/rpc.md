...
HyperBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- HyperBot owns lifecycle when `channels.signal.autoStart=true`.
...
- HyperBot spawns `imsg rpc` as a child process (legacy iMessage integration).
