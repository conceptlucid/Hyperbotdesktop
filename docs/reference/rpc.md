...
Ancient Claw integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Ancient Claw owns lifecycle when `channels.signal.autoStart=true`.
...
- Ancient Claw spawns `imsg rpc` as a child process (legacy iMessage integration).
