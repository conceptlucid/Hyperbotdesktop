...
Ancient Claw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Ancient Claw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `ancient-claw configure --section web`. It stores the key in
`~/.ancient-claw/ancient-claw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.ancient-claw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
