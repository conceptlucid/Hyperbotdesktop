...
read_when:
  ...
  - Wiring external systems into HyperBot
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your HyperBot config directory (typically `~/.hyperbot/hooks/transforms`).
...
- `hyperbot webhooks gmail setup` writes `hooks.gmail` config for `hyperbot webhooks gmail run`.
