...
- `gateway`/`node` approvals are controlled by `~/.hyperbot/exec-approvals.json`.
...
- HyperBot sets `OPENCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  HyperBot prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`hyperbot security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `hyperbot doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`hyperbot security audit` and `hyperbot doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
