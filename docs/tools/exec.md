...
- `gateway`/`node` approvals are controlled by `~/.ancient-claw/exec-approvals.json`.
...
- Ancient Claw sets `OPENCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Ancient Claw prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`ancient-claw security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `ancient-claw doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`ancient-claw security audit` and `ancient-claw doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
