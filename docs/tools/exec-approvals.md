...
- **gateway host** → `ancient-claw` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Ancient Claw also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.ancient-claw/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Ancient Claw treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.ancient-claw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `ancient-claw approvals allowlist ...`).
- `ancient-claw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `ancient-claw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Ancient Claw still rejects the `env` builtin in safe-bin
...
`~/.ancient-claw/exec-approvals.json` directly.

CLI: `ancient-claw approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Ancient Claw does not treat
...
topics, Ancient Claw preserves the topic for the approval prompt and the post-approval follow-up.
