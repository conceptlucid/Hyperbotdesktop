---
title: Lobster
summary: "Typed workflow runtime for HyperBot with resumable approval gates."
read_when:
...
Lobster is a workflow shell that lets HyperBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: HyperBot runs one Lobster tool call and gets a structured result.
...
HyperBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | hyperbot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In HyperBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the HyperBot Gateway (see the [Lobster repo](https://github.com/hyperbot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), HyperBot keeps core tools enabled. To restrict core
...
→ hyperbot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ hyperbot calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls HyperBot tools that do.
