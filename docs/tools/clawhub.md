---
summary: "ClawHub guide: public registry, native HyperBot install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **HyperBot skills and plugins**.

- Use native `hyperbot` commands to search/install/update skills and install
...
```bash
hyperbot plugins install clawhub:<package>
...
Native `hyperbot` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for HyperBot skills.
...
4. Users browse, download, and install skills in HyperBot.
...
If you want to add new capabilities to your HyperBot agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `hyperbot skills install <skill-slug>`
3. Start a new HyperBot session so it picks up the new skill.
...
## How it fits into HyperBot

Native `hyperbot skills install` installs into the active workspace `skills/`
directory. `hyperbot plugins install clawhub:...` records a normal managed
...
current working directory. If an HyperBot workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). HyperBot loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.hyperbot/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches HyperBot how to perform a
...
Interested in becoming a moderator? Ask in the HyperBot Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to HyperBot workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/hyperbot/skills` and `~/.hyperbot/skills`). This is designed to find older skill installs without extra flags.
