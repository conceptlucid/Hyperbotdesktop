---
summary: "ClawHub guide: public registry, native Ancient Claw install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Ancient Claw skills and plugins**.

- Use native `ancient-claw` commands to search/install/update skills and install
...
```bash
ancient-claw plugins install clawhub:<package>
...
Native `ancient-claw` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Ancient Claw skills.
...
4. Users browse, download, and install skills in Ancient Claw.
...
If you want to add new capabilities to your Ancient Claw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `ancient-claw skills install <skill-slug>`
3. Start a new Ancient Claw session so it picks up the new skill.
...
## How it fits into Ancient Claw

Native `ancient-claw skills install` installs into the active workspace `skills/`
directory. `ancient-claw plugins install clawhub:...` records a normal managed
...
current working directory. If an Ancient Claw workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Ancient Claw loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.ancient-claw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Ancient Claw how to perform a
...
Interested in becoming a moderator? Ask in the Ancient Claw Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Ancient Claw workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/ancient-claw/skills` and `~/.ancient-claw/skills`). This is designed to find older skill installs without extra flags.
