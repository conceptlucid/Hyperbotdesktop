...
HyperBot supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, HyperBot also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `hyperbot configure --section web`. It stores the key in
`~/.hyperbot/hyperbot.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.hyperbot/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
