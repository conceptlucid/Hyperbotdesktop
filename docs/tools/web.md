...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "HyperBot plugin SDK" });
    ```
...
If no `provider` is set, HyperBot checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  HyperBot resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `hyperbot configure --section web` or set the key directly:
