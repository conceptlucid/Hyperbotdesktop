...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Ancient Claw plugin SDK" });
    ```
...
If no `provider` is set, Ancient Claw checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Ancient Claw resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `ancient-claw configure --section web` or set the key directly:
