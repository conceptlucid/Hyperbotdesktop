...
HyperBot can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `hyperbot configure --section web` enables the bundled Firecrawl plugin automatically.
...
HyperBot always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
