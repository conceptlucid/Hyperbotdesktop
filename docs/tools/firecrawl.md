...
Ancient Claw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `ancient-claw configure --section web` enables the bundled Firecrawl plugin automatically.
...
Ancient Claw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
