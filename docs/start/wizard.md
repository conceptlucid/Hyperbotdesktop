...
CLI onboarding is the **recommended** way to set up Ancient Claw on macOS,
...
`ancient-claw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`ancient-claw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.ancient-claw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `ancient-claw doctor` first.
</Note>
...
- CLI command reference: [`ancient-claw onboard`](/cli/onboard)
