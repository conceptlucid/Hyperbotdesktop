...
CLI onboarding is the **recommended** way to set up HyperBot on macOS,
...
`hyperbot dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`hyperbot configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.hyperbot/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `hyperbot doctor` first.
</Note>
...
- CLI command reference: [`hyperbot onboard`](/cli/onboard)
