---
summary: "Optional Docker-based setup and onboarding for Ancient Claw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Ancient Claw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export OPENCLAW_IMAGE="ghcr.io/ancient-claw/ancient-claw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/ancient-claw/ancient-claw/pkgs/container/ancient-claw).
  ...
  <Step title="Complete onboarding">
    ...
    `ancient-claw-gateway` directly. `ancient-claw-cli` is for commands you run after
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm ancient-claw-cli channels login

    # Telegram
    docker compose run --rm ancient-claw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm ancient-claw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t ancient-claw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node ancient-claw-gateway \
...
<Note>
Because `ancient-claw-cli` shares `ancient-claw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d ancient-claw-gateway`, run onboarding
and setup-time config writes through `ancient-claw-gateway` with
...
```bash
docker compose exec ancient-claw-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.ancient-claw` and
`OPENCLAW_WORKSPACE_DIR` to `/home/node/.ancient-claw/workspace`, so those paths
...
and rolling file logs under `/tmp/ancient-claw/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/ancient-claw/ancient-claw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/ancient-claw/ancient-claw/blob/main/scripts/shell-helpers/README.md).
...
<AccordionGroup>
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm ancient-claw-cli gateway probe
    docker compose run -T --rm ancient-claw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `ancient-claw-cli` uses `network_mode: "service:ancient-claw-gateway"` so CLI
    ...
    `no-new-privileges` on `ancient-claw-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.ancient-claw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/ancient-claw-config /path/to/ancient-claw-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export OPENCLAW_HOME_VOLUME="ancient-claw_home"`
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/ancient-claw/ancient-claw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Ancient Claw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm ancient-claw-cli config set gateway.mode local
    docker compose run --rm ancient-claw-cli config set gateway.bind lan
    docker compose run --rm ancient-claw-cli devices list --url ws://127.0.0.1:18789
    ```
