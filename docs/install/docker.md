---
summary: "Optional Docker-based setup and onboarding for HyperBot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run HyperBot on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export OPENCLAW_IMAGE="ghcr.io/hyperbot/hyperbot:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/hyperbot/hyperbot/pkgs/container/hyperbot).
  ...
  <Step title="Complete onboarding">
    ...
    `hyperbot-gateway` directly. `hyperbot-cli` is for commands you run after
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm hyperbot-cli channels login

    # Telegram
    docker compose run --rm hyperbot-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm hyperbot-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t hyperbot:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node hyperbot-gateway \
...
<Note>
Because `hyperbot-cli` shares `hyperbot-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d hyperbot-gateway`, run onboarding
and setup-time config writes through `hyperbot-gateway` with
...
```bash
docker compose exec hyperbot-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.hyperbot` and
`OPENCLAW_WORKSPACE_DIR` to `/home/node/.hyperbot/workspace`, so those paths
...
and rolling file logs under `/tmp/hyperbot/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/hyperbot/hyperbot/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/hyperbot/hyperbot/blob/main/scripts/shell-helpers/README.md).
...
<AccordionGroup>
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm hyperbot-cli gateway probe
    docker compose run -T --rm hyperbot-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `hyperbot-cli` uses `network_mode: "service:hyperbot-gateway"` so CLI
    ...
    `no-new-privileges` on `hyperbot-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.hyperbot`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/hyperbot-config /path/to/hyperbot-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export OPENCLAW_HOME_VOLUME="hyperbot_home"`
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/hyperbot/hyperbot/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    HyperBot runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm hyperbot-cli config set gateway.mode local
    docker compose run --rm hyperbot-cli config set gateway.bind lan
    docker compose run --rm hyperbot-cli devices list --url ws://127.0.0.1:18789
    ```
