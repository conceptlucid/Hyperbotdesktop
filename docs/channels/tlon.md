...
Tlon is a decentralized messenger built on Urbit. HyperBot connects to your Urbit ship and can
...
```bash
hyperbot plugins install @hyperbot/tlon
```

Local checkout (when running from a git repo):

```bash
hyperbot plugins install ./extensions/tlon
```
...
By default, HyperBot blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `hyperbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, HyperBot replies in-thread.
