...
Tlon is a decentralized messenger built on Urbit. Ancient Claw connects to your Urbit ship and can
...
```bash
ancient-claw plugins install @ancient-claw/tlon
```

Local checkout (when running from a git repo):

```bash
ancient-claw plugins install ./extensions/tlon
```
...
By default, Ancient Claw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `ancient-claw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Ancient Claw replies in-thread.
