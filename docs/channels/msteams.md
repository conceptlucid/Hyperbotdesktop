...
```bash
hyperbot plugins install @hyperbot/msteams
```

Local checkout (when running from a git repo):

```bash
hyperbot plugins install ./extensions/msteams
```
...
3. Configure HyperBot with those credentials.
...
- Talk to HyperBot via Teams DMs, group chats, or channels.
...
- On startup, HyperBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.hyperbot/hyperbot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring HyperBot, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `hyperbot-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `hyperbot plugins install @hyperbot/msteams`
   - From a local checkout: `hyperbot plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "HyperBot in Teams", full: "HyperBot in Teams" },
...
HyperBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, HyperBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/HyperBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

HyperBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `hyperbot message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.hyperbot/msteams-polls.json`.
...
```bash
hyperbot message send --channel msteams \
...
hyperbot message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
hyperbot message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
hyperbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
hyperbot message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
