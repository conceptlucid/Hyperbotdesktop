...
read_when:
  - Setting up Twitch chat integration for Ancient Claw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Ancient Claw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
ancient-claw plugins install @ancient-claw/twitch
```

Local checkout (when running from a git repo):

```bash
ancient-claw plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "ancient-claw", // Bot's Twitch account
