...
read_when:
  - Setting up Twitch chat integration for HyperBot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. HyperBot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
hyperbot plugins install @hyperbot/twitch
```

Local checkout (when running from a git repo):

```bash
hyperbot plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "hyperbot", // Bot's Twitch account
