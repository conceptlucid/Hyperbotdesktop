...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/ancient-claw`)
...
If the page fails to load, start the Gateway first: `ancient-claw gateway`.
...
created. Re-run `ancient-claw devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `ancient-claw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.ancient-claw/ancient-claw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop ancient-claw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Ancient Claw
...
Ancient Claw **blocks** Control UI connections without device identity.
