...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/hyperbot`)
...
If the page fails to load, start the Gateway first: `hyperbot gateway`.
...
created. Re-run `hyperbot devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `hyperbot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.hyperbot/hyperbot.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop hyperbot`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. HyperBot
...
HyperBot **blocks** Control UI connections without device identity.
