...
- Turn delivery on:
  ...
  - or start with `ancient-claw tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `ancient-claw tui`.
...
- Check the Gateway logs: `ancient-claw logs --follow`.
- Confirm the agent can run: `ancient-claw status` and `ancient-claw models status`.
...
- No agents in picker: check `ancient-claw agents list` and your routing config.
