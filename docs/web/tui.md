...
- Turn delivery on:
  ...
  - or start with `hyperbot tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `hyperbot tui`.
...
- Check the Gateway logs: `hyperbot logs --follow`.
- Confirm the agent can run: `hyperbot status` and `hyperbot models status`.
...
- No agents in picker: check `hyperbot agents list` and your routing config.
