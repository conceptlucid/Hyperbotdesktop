...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Ancient Claw
...
Running Ancient Claw via Node with `tsx` fails at startup with:

```
[ancient-claw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `ancient-claw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch ancient-claw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node ancient-claw.mjs status` works on Node 25.
