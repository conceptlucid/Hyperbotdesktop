...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in HyperBot
...
Running HyperBot via Node with `tsx` fails at startup with:

```
[hyperbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `hyperbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch hyperbot.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node hyperbot.mjs status` works on Node 25.
