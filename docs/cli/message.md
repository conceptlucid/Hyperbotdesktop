---
summary: "CLI reference for `hyperbot message` (send + channel actions)"
read_when:
...
```
hyperbot message <subcommand> [flags]
```
...
- On cache miss, HyperBot will attempt a live directory lookup when the provider supports it.
...
- `hyperbot message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, HyperBot does not force a `default` account SecretRef scope
...
```
hyperbot message send --channel discord \
...
```
hyperbot message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
hyperbot message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
hyperbot message poll --channel msteams \
...
```
hyperbot message react --channel slack \
...
```
hyperbot message react --channel signal \
...
```
hyperbot message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
hyperbot message send --channel telegram --target @mychat \
