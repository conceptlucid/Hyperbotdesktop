---
summary: "CLI reference for `ancient-claw message` (send + channel actions)"
read_when:
...
```
ancient-claw message <subcommand> [flags]
```
...
- On cache miss, Ancient Claw will attempt a live directory lookup when the provider supports it.
...
- `ancient-claw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Ancient Claw does not force a `default` account SecretRef scope
...
```
ancient-claw message send --channel discord \
...
```
ancient-claw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
ancient-claw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
ancient-claw message poll --channel msteams \
...
```
ancient-claw message react --channel slack \
...
```
ancient-claw message react --channel signal \
...
```
ancient-claw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
ancient-claw message send --channel telegram --target @mychat \
