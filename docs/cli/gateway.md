---
summary: "HyperBot Gateway CLI (`hyperbot gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is HyperBot’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `hyperbot gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.hyperbot/hyperbot.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
```bash
hyperbot gateway health --url ws://127.0.0.1:18789
```
...
hyperbot gateway status --require-rpc
```
...
```bash
hyperbot gateway probe --ssh user@gateway-host
```
...
```bash
hyperbot gateway call status
hyperbot gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
`gateway discover` scans for Gateway beacons (`_hyperbot-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `hyperbot.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
hyperbot gateway discover --timeout 4000
hyperbot gateway discover --json | jq '.beacons[].wsUrl'
```
