---
summary: "Ancient Claw Gateway CLI (`ancient-claw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Ancient Claw’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `ancient-claw gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.ancient-claw/ancient-claw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
```bash
ancient-claw gateway health --url ws://127.0.0.1:18789
```
...
ancient-claw gateway status --require-rpc
```
...
ancient-claw gateway probe --json
```
...
```bash
ancient-claw gateway probe --ssh user@gateway-host
```
...
```bash
ancient-claw gateway call status
ancient-claw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
`gateway discover` scans for Gateway beacons (`_ancient-claw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `ancient-claw.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
ancient-claw gateway discover --timeout 4000
ancient-claw gateway discover --json | jq '.beacons[].wsUrl'
```
