---
summary: "CLI reference for `ancient-claw status` (diagnostics, probes, usage snapshots)"
read_when:
...
- Update info surfaces in the Overview; if an update is available, status prints a hint to run `ancient-claw update` (see [Updating](/install/updating)).
