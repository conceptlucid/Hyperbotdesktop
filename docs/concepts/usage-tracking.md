...
- `/usage cost` in chats: local cost summary aggregated from HyperBot session logs.
- CLI: `hyperbot status --usage` prints a full per-provider breakdown.
- CLI: `hyperbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
