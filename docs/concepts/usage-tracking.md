...
- `/usage cost` in chats: local cost summary aggregated from Ancient Claw session logs.
- CLI: `ancient-claw status --usage` prints a full per-provider breakdown.
- CLI: `ancient-claw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
