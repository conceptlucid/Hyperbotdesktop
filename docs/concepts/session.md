...
HyperBot treats **one direct-chat session per agent** as primary. Direct chats collapse to `agent:<agentId>:<mainKey>` (default `main`), while group/channel chats get their own keys. `session.mainKey` is honored.
...
- You can verify your DM settings with `hyperbot security audit` (see [security](/cli/security)).

## Gateway is the source of truth

All session state is **owned by the gateway** (the “master” HyperBot). UI clients (macOS app, WebChat, etc.) must query the gateway for session lists and token counts instead of reading local files.
...
- On the **gateway host**:
  - Store file: `~/.hyperbot/agents/<agentId>/sessions/sessions.json` (per agent).
- Transcripts: `~/.hyperbot/agents/<agentId>/sessions/<SessionId>.jsonl` (Telegram topic sessions use `.../<SessionId>-topic-<threadId>.jsonl`).
- The store is a map `sessionKey -> { sessionId, updatedAt, ... }`. Deleting entries is safe; they are recreated on demand.
- Group entries may include `displayName`, `channel`, `subject`, `room`, and `space` to label sessions in UIs.
- Session entries include `origin` metadata (label + routing hints) so UIs can explain where a session came from.
- HyperBot does **not** read legacy Pi/Tau session folders.

## Maintenance

HyperBot applies session-store maintenance to keep `sessions.json` and transcript artifacts bounded over time.
...
Maintenance runs during session-store writes, and you can trigger it on demand with `hyperbot sessions cleanup`.
...
- many transcript/archive artifacts in `~/.hyperbot/agents/<agentId>/sessions/`
...
- run `hyperbot sessions cleanup --dry-run --json` after config changes to verify projected impact before enforcing
...
```bash
hyperbot sessions cleanup --dry-run
hyperbot sessions cleanup --enforce
```

## Session pruning

HyperBot trims **old tool results** from the in-memory context right before LLM calls by default.
This does **not** rewrite JSONL history. See [/concepts/session-pruning](/concepts/session-pruning).

## Pre-compaction memory flush

When a session nears auto-compaction, HyperBot can run a **silent memory flush**
...
- Legacy idle-only: if you set `session.idleMinutes` without any `session.reset`/`resetByType` config, HyperBot stays in idle-only mode for backward compatibility.
...
- `hyperbot status` — shows store path and recent sessions.
- `hyperbot sessions --json` — dumps every entry (filter with `--active <minutes>`).
- `hyperbot gateway call sessions.list --params '{}'` — fetch sessions from the running gateway (use `--url`/`--token` for remote gateway access).
...
- Send `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop hyperbot`) to abort the current run, clear queued followups for that session, and stop any sub-agent runs spawned from it (the reply includes the stopped count).
