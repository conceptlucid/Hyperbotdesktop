...
read_when:
  - You want to understand how Ancient Claw assembles model context
...
A **context engine** controls how Ancient Claw builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Ancient Claw ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.ancient-claw/ancient-claw.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Ancient Claw plugin. Install
...
ancient-claw plugins install -l ./my-context-engine
```
...
Every time Ancient Claw runs a model prompt, the context engine participates at
...
Ancient Claw currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Ancient Claw
...
The built-in `legacy` engine preserves Ancient Claw's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Ancient Claw uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Ancient Claw disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Ancient Claw automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `ancient-claw/plugin-sdk/core` to use
  Ancient Claw's built-in compaction behavior.
...
Ancient Claw resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Ancient Claw's built-in summarization. Plugin engines can implement
...
- Use `ancient-claw doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Ancient Claw
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `ancient-claw plugins install -l ./my-engine` to link a
