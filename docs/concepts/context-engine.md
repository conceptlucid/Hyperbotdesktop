...
read_when:
  - You want to understand how HyperBot assembles model context
...
A **context engine** controls how HyperBot builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

HyperBot ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.hyperbot/hyperbot.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other HyperBot plugin. Install
...
hyperbot plugins install -l ./my-context-engine
```
...
Every time HyperBot runs a model prompt, the context engine participates at
...
HyperBot currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. HyperBot
...
The built-in `legacy` engine preserves HyperBot's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. HyperBot uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. HyperBot disables Pi's built-in
...
`ownsCompaction: false` does **not** mean HyperBot automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `hyperbot/plugin-sdk/core` to use
  HyperBot's built-in compaction behavior.
...
HyperBot resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to HyperBot's built-in summarization. Plugin engines can implement
...
- Use `hyperbot doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, HyperBot
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `hyperbot plugins install -l ./my-engine` to link a
