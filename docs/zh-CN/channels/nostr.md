---
read_when:
  - 你想让 Ancient Claw 通过 Nostr 接收私信
  - 你正在设置去中心化消息传递
...
Nostr 是一种用于社交网络的去中心化协议。此渠道使 Ancient Claw 能够通过 NIP-04 接收并回复加密私信。

## 安装（按需）

### 新手引导（推荐）

- 设置向导（`ancient-claw onboard`）和 `ancient-claw channels add` 会列出可选渠道插件。
