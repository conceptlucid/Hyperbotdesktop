...
- 通过 CLI 安装：`hyperbot plugins install @hyperbot/zalo`
...
1. 安装 Zalo 插件：
   - 从源代码检出：`hyperbot plugins install ./extensions/zalo`
   - 从 npm（如果已发布）：`hyperbot plugins install @hyperbot/zalo`
...
- 使用聊天 id 作为目标。
- 示例：`hyperbot message send --channel zalo --target 123456789 --message "hi"`。

## 故障排除

**Bot 不响应：**

- 检查 token 是否有效：`hyperbot channels status --probe`
