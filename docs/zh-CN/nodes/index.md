...
macOS 也可以在**节点模式**下运行：菜单栏应用连接到 Gateway 网关的 WS 服务器，并将其本地 canvas/camera 命令作为节点暴露（因此 `ancient-claw nodes …` 可以针对这台 Mac 工作）。
...
- `node.pair.*`（CLI：`ancient-claw nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- **批准**：通过 `~/.ancient-claw/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.ancient-claw/ancient-claw.json`）。
- `ancient-claw node run` 读取 `OPENCLAW_GATEWAY_TOKEN` 进行认证。
...
- 在 `ancient-claw node run` / `ancient-claw node install` 上使用 `--display-name`（持久化在节点上的 `~/.ancient-claw/node.json` 中）。
- `ancient-claw nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
批准存储在节点主机的 `~/.ancient-claw/exec-approvals.json` 中。
...
ancient-claw nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.ancient-claw/exec-approvals.json`）。
...
Ancient Claw 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.ancient-claw/node.json` 中。
- Exec 批准通过 `~/.ancient-claw/exec-approvals.json` 在本地执行
...
- macOS 菜单栏应用作为节点连接到 Gateway 网关 WS 服务器（因此 `ancient-claw nodes …` 可以针对这台 Mac 工作）。
