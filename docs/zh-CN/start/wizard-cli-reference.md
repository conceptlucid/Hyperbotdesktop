---
read_when:
  - 你需要了解 `ancient-claw onboard` 的详细行为
...
本页是 `ancient-claw onboard` 的完整参考。
...
<Steps>
  <Step title="现有配置检测">
    - 如果 `~/.ancient-claw/ancient-claw.json` 存在，可选择 Keep、Modify 或 Reset。
    - 重新运行向导不会清除任何内容，除非你明确选择 Reset（或传递 `--reset`）。
    - CLI `--reset` 默认作用于 `config+creds+sessions`；使用 `--reset-scope full` 还会删除工作区。
    - 如果配置无效或包含旧版键，向导会停止，并要求你先运行 `ancient-claw doctor` 再继续。
  ...
  <Step title="工作区">
    - 默认值为 `~/.ancient-claw/workspace`（可配置）。
  ...
  <Step title="渠道">
    ...
    - 私信安全：默认是配对。首次私信会发送一个代码；通过
      `ancient-claw pairing approve <channel> <code>` 批准，或使用 allowlist。
  </Step>
  <Step title="守护进程安装">
    - macOS：LaunchAgent
  ...
  <Step title="健康检查">
    - 启动 Gateway 网关（如有需要），并运行 `ancient-claw health`。
    - `ancient-claw status --deep` 会在状态输出中添加 Gateway 网关健康探测。
  </Step>
  <Step title="Skills">
    - 读取可用的 Skills 并检查要求。
...
- OAuth 凭证：`~/.ancient-claw/credentials/oauth.json`
- Auth profile（API 密钥 + OAuth）：`~/.ancient-claw/agents/<agentId>/agent/auth-profiles.json`
...
`~/.ancient-claw/credentials/oauth.json`（或 `$OPENCLAW_STATE_DIR/credentials/oauth.json`）
到 Gateway 网关主机。
</Note>
...
WhatsApp 凭证位于 `~/.ancient-claw/credentials/whatsapp/<accountId>/`。
...
- 下载适当的发布资源
- 将其存储在 `~/.ancient-claw/tools/signal-cli/<version>/`
...
- 命令参考：[`ancient-claw onboard`](/cli/onboard)
