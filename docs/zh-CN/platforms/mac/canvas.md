...
- `~/Library/Application Support/HyperBot/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `hyperbot-canvas://<session>/<path>`

示例：

- `hyperbot-canvas://main/` → `<canvasRoot>/main/index.html`
- `hyperbot-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `hyperbot-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
hyperbot nodes canvas present --node <id>
hyperbot nodes canvas navigate --node <id> --url "/"
hyperbot nodes canvas eval --node <id> --js "document.title"
hyperbot nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__hyperbot__/a2ui/
```
...
hyperbot nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
hyperbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "hyperbot://agent?message=Review%20this%20design";
```
