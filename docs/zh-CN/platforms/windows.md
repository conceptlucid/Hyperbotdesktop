...
推荐在 Windows 上**通过 WSL2** 运行 HyperBot（推荐 Ubuntu）。CLI + Gateway 网关在 Linux 内运行，这能保持运行时一致，并使
...
- 本地 CLI 用法，例如 `hyperbot --version`、`hyperbot doctor` 和 `hyperbot plugins list --json`
...
- 除非你传递 `--skip-health`，否则 `hyperbot onboard --non-interactive` 仍然要求本地 Gateway 网关可访问
- `hyperbot onboard --non-interactive --install-daemon` 和 `hyperbot gateway install` 会优先尝试 Windows Scheduled Tasks
- 如果拒绝创建 Scheduled Task，HyperBot 会回退到每用户 Startup 文件夹登录项，并立即启动 Gateway 网关
- 如果 `schtasks` 本身卡住或停止响应，HyperBot 现在会快速中止该路径并回退，而不是无限挂起
