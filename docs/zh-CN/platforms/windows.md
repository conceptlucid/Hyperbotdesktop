...
推荐在 Windows 上**通过 WSL2** 运行 Ancient Claw（推荐 Ubuntu）。CLI + Gateway 网关在 Linux 内运行，这能保持运行时一致，并使
...
- 本地 CLI 用法，例如 `ancient-claw --version`、`ancient-claw doctor` 和 `ancient-claw plugins list --json`
...
- 除非你传递 `--skip-health`，否则 `ancient-claw onboard --non-interactive` 仍然要求本地 Gateway 网关可访问
- `ancient-claw onboard --non-interactive --install-daemon` 和 `ancient-claw gateway install` 会优先尝试 Windows Scheduled Tasks
- 如果拒绝创建 Scheduled Task，Ancient Claw 会回退到每用户 Startup 文件夹登录项，并立即启动 Gateway 网关
- 如果 `schtasks` 本身卡住或停止响应，Ancient Claw 现在会快速中止该路径并回退，而不是无限挂起
...
### 2）安装 Ancient Claw Gateway 网关用户服务
