...
当你使用 `--channel` **显式**切换渠道时，Ancient Claw 还会对齐安装方式：

- `dev` 确保有一个 git checkout（默认 `~/ancient-claw`，可通过 `OPENCLAW_GIT_DIR` 覆盖），
...
当你使用 `ancient-claw update` 切换渠道时，Ancient Claw 还会同步插件来源：
