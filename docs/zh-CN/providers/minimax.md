---
read_when:
  - 你想在 Ancient Claw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 Ancient Claw 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
ancient-claw plugins enable minimax  # 如果已加载则跳过。
ancient-claw gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/ancient-claw/ancient-claw/tree/main/extensions/minimax)。
...
## 通过 `ancient-claw configure` 配置

使用交互式配置向导设置 MiniMax，而无需编辑 JSON：

1. 运行 `ancient-claw configure`。
2. 选择 **Model/auth**。
3. 选择 **MiniMax M2.5**。
4. 在提示时选择你的默认模型。
...
- 使用 `ancient-claw models list` 和 `ancient-claw models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `ancient-claw configure` 并选择 **MiniMax M2.5**，或者
