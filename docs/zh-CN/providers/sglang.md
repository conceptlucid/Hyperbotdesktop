---
read_when:
  - 你想让 HyperBot 连接本地 SGLang 服务器运行
  - 你想通过自己的模型使用兼容 OpenAI 的 `/v1` 端点
summary: 让 HyperBot 与 SGLang 一起运行（兼容 OpenAI 的自托管服务器）
title: SGLang
x-i18n:
...
HyperBot 可以使用 `openai-completions` API 连接到 SGLang。

当你通过 `SGLANG_API_KEY` 选择加入时，HyperBot 还可以**自动发现**
...
定义 `models.providers.sglang` 时，HyperBot 将查询：
