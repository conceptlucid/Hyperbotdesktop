...
summary: Ancient Claw 插件/扩展：发现、配置与安全
title: 插件
x-i18n:
...
- 原生 **Ancient Claw 插件**（`ancient-claw.plugin.json` + 运行时模块），或
- 兼容的 **bundle**（`.codex-plugin/plugin.json` 或 `.claude-plugin/plugin.json`）

两者都会显示在 `ancient-claw plugins` 下，但只有原生 Ancient Claw 插件会在进程内执行运行时代码。

大多数情况下，当你想要某项核心 Ancient Claw 尚未内置的功能时，就会使用插件（或者你希望将可选功能保留在主安装之外）。
...
裸规格和 `@latest` 会停留在稳定通道。如果 npm 将其中任一解析为预发布版本，Ancient Claw 会停止并要求你通过预发布标签（例如 `@beta`/`@rc`）或精确的预发布版本显式选择加入。
...
Ancient Claw 的插件系统有四层：

1. **清单 + 发现**
   Ancient Claw 会从已配置路径、工作区根目录、全局扩展根目录以及随附扩展中查找候选插件。发现过程会先读取原生 `ancient-claw.plugin.json` 清单以及受支持的 bundle 清单。
2. **启用 + 验证**
   核心会决定某个已发现插件是启用、禁用、阻止，还是被选用于某个独占插槽（例如内存）。
3. **运行时加载**
   原生 Ancient Claw 插件通过 jiti 在进程内加载，并将功能注册到一个中央注册表中。兼容的 bundle 会被规范化为注册表记录，而不会导入运行时代码。
4. **表面消费**
   Ancient Claw 的其余部分会读取注册表，以公开工具、渠道、提供商设置、hooks、HTTP 路由、CLI 命令和服务。
...
这种拆分让 Ancient Claw 能够在完整运行时激活之前，就验证配置、解释缺失/被禁用的插件，并构建 UI/schema 提示。

## 兼容的 bundle

Ancient Claw 还识别两种兼容的外部 bundle 布局：
...
目前，Ancient Claw 将这些内容视为 **能力包**，而不是原生运行时插件：

- 当前支持：捆绑的 `skills`
- 当前支持：Claude `commands/` markdown 根目录，映射到常规 Ancient Claw 技能加载器
- 当前支持：Claude bundle `settings.json` 默认值，用于嵌入式 Pi 智能体设置（会清理 shell override 键）
- 当前支持：Cursor `.cursor/commands/*.md` 根目录，映射到常规 Ancient Claw 技能加载器
- 当前支持：使用 Ancient Claw hook-pack 布局的 Codex bundle hook 目录（`HOOK.md` + `handler.ts`/`handler.js`）
...
Bundle hook 支持仅限于常规 Ancient Claw hook 目录格式（在声明的 hook 根目录下使用 `HOOK.md` 加上 `handler.ts`/`handler.js`）。
供应商特定的 shell/JSON hook 运行时，包括 Claude `hooks.json`，当前仅会被检测，不会被直接执行。

## 执行模型

原生 Ancient Claw 插件与 Gateway 网关 **在同一进程内** 运行。它们不会进行沙箱隔离。已加载的原生插件与核心代码处于相同的进程级信任边界。

这意味着：

- 原生插件可以注册工具、网络处理器、hooks 和服务
- 原生插件中的 bug 可能导致 gateway 崩溃或不稳定
- 恶意原生插件等同于在 Ancient Claw 进程内部执行任意代码

兼容的 bundle 默认更安全，因为 Ancient Claw 目前将它们视为元数据/内容包。在当前版本中，这主要意味着捆绑的技能。
...
- Microsoft Teams 自 `2026.1.15` 起仅以插件形式提供；如果你使用 Teams，请安装 `@ancient-claw/msteams`。
...
- [Voice Call](/plugins/voice-call) — `@ancient-claw/voice-call`
- [Zalo Personal](/plugins/zalouser) — `@ancient-claw/zalouser`
- [Matrix](/channels/matrix) — `@ancient-claw/matrix`
- [Nostr](/channels/nostr) — `@ancient-claw/nostr`
- [Zalo](/channels/zalo) — `@ancient-claw/zalo`
- [Microsoft Teams](/channels/msteams) — `@ancient-claw/msteams`
...
原生 Ancient Claw 插件与 Gateway 网关 **在同一进程内** 运行，因此应将其视为受信任代码。
...
Ancient Claw 仍然负责通用的智能体循环、故障切换、转录处理和工具策略。这些 hooks 是 provider 特定行为的接缝，而无需整个自定义推理传输层。
...
对于 model/provider 插件，Ancient Claw 大致按以下顺序使用 hooks：

1. `catalog`
   在生成 `models.json` 期间，将 provider 配置发布到 `models.providers` 中。
2. 内置/已发现模型查找
   Ancient Claw 会先尝试常规注册表/目录路径。
...
如果 provider 需要完全自定义的线协议或自定义请求执行器，那属于另一类扩展。这些 hooks 适用于仍运行在 Ancient Claw 常规推理循环上的 provider 行为。
...
- OpenRouter 使用 `catalog` 以及 `resolveDynamicModel` 和 `prepareDynamicModel`，因为该 provider 是透传型的，可能会在 Ancient Claw 静态目录更新之前暴露新的 model id。
...
Manifest 是控制面的事实来源。Ancient Claw 用它来：
...
编写插件时，请使用 SDK 子路径，而不是单体式 `ancient-claw/plugin-sdk` 导入：

- `ancient-claw/plugin-sdk/core` 用于通用插件 API、provider 身份验证类型和共享辅助工具。
- `ancient-claw/plugin-sdk/compat` 用于比 `core` 需要更广泛共享运行时辅助工具的捆绑/内部插件代码。
- `ancient-claw/plugin-sdk/telegram` 用于 Telegram 渠道插件。
- `ancient-claw/plugin-sdk/discord` 用于 Discord 渠道插件。
- `ancient-claw/plugin-sdk/slack` 用于 Slack 渠道插件。
- `ancient-claw/plugin-sdk/signal` 用于 Signal 渠道插件。
- `ancient-claw/plugin-sdk/imessage` 用于 iMessage 渠道插件。
- `ancient-claw/plugin-sdk/whatsapp` 用于 WhatsApp 渠道插件。
- `ancient-claw/plugin-sdk/line` 用于 LINE 渠道插件。
- `ancient-claw/plugin-sdk/msteams` 用于捆绑的 Microsoft Teams 插件表面。
- 也提供捆绑扩展专用子路径：
  `ancient-claw/plugin-sdk/acpx`、`ancient-claw/plugin-sdk/bluebubbles`、
  `ancient-claw/plugin-sdk/copilot-proxy`、`ancient-claw/plugin-sdk/device-pair`、
  `ancient-claw/plugin-sdk/diagnostics-otel`、`ancient-claw/plugin-sdk/diffs`、
  `ancient-claw/plugin-sdk/feishu`、`ancient-claw/plugin-sdk/googlechat`、
  `ancient-claw/plugin-sdk/irc`、`ancient-claw/plugin-sdk/llm-task`、
  `ancient-claw/plugin-sdk/lobster`、`ancient-claw/plugin-sdk/matrix`、
  `ancient-claw/plugin-sdk/mattermost`、`ancient-claw/plugin-sdk/memory-core`、
  `ancient-claw/plugin-sdk/memory-lancedb`、
  `ancient-claw/plugin-sdk/minimax-portal-auth`、
  `ancient-claw/plugin-sdk/nextcloud-talk`、`ancient-claw/plugin-sdk/nostr`、
  `ancient-claw/plugin-sdk/open-prose`、`ancient-claw/plugin-sdk/phone-control`、
  `ancient-claw/plugin-sdk/qwen-portal-auth`、`ancient-claw/plugin-sdk/synology-chat`、
  `ancient-claw/plugin-sdk/talk-voice`、`ancient-claw/plugin-sdk/test-utils`、
  `ancient-claw/plugin-sdk/thread-ownership`、`ancient-claw/plugin-sdk/tlon`、
  `ancient-claw/plugin-sdk/twitch`、`ancient-claw/plugin-sdk/voice-call`、
  `ancient-claw/plugin-sdk/zalo` 和 `ancient-claw/plugin-sdk/zalouser`。
...
`catalog.run(...)` 返回与 Ancient Claw 写入
...
`catalog.order` 控制插件目录相对于 Ancient Claw 内置隐式 provider 的合并时机：
...
- 如果同时注册了 `catalog` 和 `discovery`，Ancient Claw 会使用 `catalog`

兼容性说明：

- `ancient-claw/plugin-sdk` 仍支持现有外部插件。
...
- 只读命令路径，例如 `ancient-claw status`、`ancient-claw status --all`、
  `ancient-claw channels status`、`ancient-claw channels resolve` 以及 Doctor/配置修复流程，不应仅为了描述配置就必须实体化运行时凭据。
...
- `<workspace>/.ancient-claw/extensions/*.ts`
- `<workspace>/.ancient-claw/extensions/*/index.ts`

3. 全局扩展

- `~/.ancient-claw/extensions/*.ts`
- `~/.ancient-claw/extensions/*/index.ts`

4. 捆绑扩展（随 Ancient Claw 一起提供；默认开启/关闭混合）

- `<ancient-claw>/extensions/*`

许多捆绑的 provider 插件默认启用，这样模型目录/运行时 hooks 无需额外设置即可使用。其他插件仍需要通过 `plugins.entries.<id>.enabled` 或
`ancient-claw plugins enable <id>` 显式启用。
...
- 如果 `plugins.allow` 为空且可发现非捆绑插件，Ancient Claw 会在启动时记录一条警告，其中包含 plugin id 和来源。
- 候选路径在被接受进入发现流程前会经过安全检查。当出现以下情况时，Ancient Claw 会阻止候选项：
...
每个原生 Ancient Claw 插件都必须在其根目录中包含一个 `ancient-claw.plugin.json` 文件。
...
插件目录可以包含带有 `ancient-claw.extensions` 的 `package.json`：
...
安全护栏：每个 `ancient-claw.extensions` 条目在解析符号链接后都必须保持在插件目录内。逃逸出包目录的条目会被拒绝。

安全说明：`ancient-claw plugins install` 会使用
`npm install --ignore-scripts` 安装插件依赖（不会运行生命周期脚本）。
请保持插件依赖树为“纯 JS/TS”，并避免需要 `postinstall` 构建的包。

可选项：`ancient-claw.setupEntry` 可以指向一个轻量级、仅用于 setup 的模块。
当 Ancient Claw 需要为一个已禁用的渠道插件提供 setup 表面，或某个渠道插件虽然已启用但仍未配置时，它会加载 `setupEntry`，而不是完整插件入口。
当你的主插件入口还连接了工具、hooks 或其他仅运行时代码时，这能让启动和 setup 更轻量。

### 渠道目录元数据

渠道插件可以通过 `ancient-claw.channel` 公布 setup/discovery 元数据，并通过 `ancient-claw.install` 公布安装提示。这使核心目录保持无数据化。

示例：

```json
{
  "name": "@ancient-claw/nextcloud-talk",
  "ancient-claw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@ancient-claw/nextcloud-talk",
...
Ancient Claw 还可以合并 **外部渠道目录**（例如 MPM registry 导出）。
将 JSON 文件放到以下任一位置：

- `~/.ancient-claw/mpm/plugins.json`
- `~/.ancient-claw/mpm/catalog.json`
- `~/.ancient-claw/plugins/catalog.json`

或者将 `OPENCLAW_PLUGIN_CATALOG_PATHS`（或 `OPENCLAW_MPM_CATALOG_PATHS`）指向一个或多个 JSON 文件（用逗号/分号/`PATH` 分隔）。
每个文件应包含
`{ "entries": [ { "name": "@scope/pkg", "ancient-claw": { "channel": {...}, "install": {...} } } ] }`。
...
如果插件导出 `id`，Ancient Claw 会使用它，但如果它与配置的 id 不匹配，则会发出警告。
...
- 原生插件配置使用嵌入在 `ancient-claw.plugin.json` 中的 JSON Schema（`configSchema`）进行验证。
- 兼容的 bundle 当前不公开原生 Ancient Claw 配置 schema。
...
Ancient Claw 会保留已禁用插件的配置，因此重新启用它们不会造成破坏。
...
Ancient Claw 会在运行时根据已发现的插件增强 `uiHints`：
...
ancient-claw plugins install <path>                 # 将本地文件/目录复制到 ~/.ancient-claw/extensions/<id>
ancient-claw plugins install ./extensions/voice-call # 支持相对路径
ancient-claw plugins install ./plugin.tgz           # 从本地 tarball 安装
ancient-claw plugins install ./plugin.zip           # 从本地 zip 安装
ancient-claw plugins install -l ./extensions/voice-call # link（不复制），用于开发
ancient-claw plugins install @ancient-claw/voice-call # 从 npm 安装
ancient-claw plugins install @ancient-claw/voice-call --pin # 存储精确解析后的 name@version
...
`ancient-claw plugins list` 会将顶层格式显示为 `ancient-claw` 或 `bundle`。
详细列表/info 输出还会显示 bundle 子类型（`codex` 或 `claude`）以及已检测到的 bundle 能力。

`plugins update` 仅适用于在 `plugins.installs` 下跟踪的 npm 安装。
如果更新之间存储的完整性元数据发生变化，Ancient Claw 会发出警告并要求确认（使用全局 `--yes` 可绕过提示）。

插件也可以注册自己的顶层命令（例如：`ancient-claw voicecall`）。
...
```ts
import { delegateCompactionToRuntime } from "ancient-claw/plugin-sdk";
...
- 插件管理的 hooks 会在 `ancient-claw hooks list` 中显示为 `plugin:<id>`。
- 你不能通过 `ancient-claw hooks` 启用/禁用插件管理的 hooks；应改为启用/禁用插件本身。
...
- 禁用后，Ancient Claw 会阻止 `before_prompt_build`，并忽略旧版 `before_agent_start` 返回的提示词变更字段，同时保留旧版 `modelOverride` 和 `providerOverride`。
...
插件可以注册 **模型 providers**，从而让用户能够在 Ancient Claw 内完成 OAuth 或 API 密钥设置、在新手引导/模型选择器中显示 provider 设置，并参与隐式 provider 发现。
...
2. **非交互式设置**
   `auth[].runNonInteractive(ctx)` 处理 `ancient-claw onboard --non-interactive`，且不进行提示。当 provider 需要超出内置简单 API 密钥路径之外的自定义无头设置时，请使用它。
3. **向导集成**
   `wizard.setup` 会向 `ancient-claw onboard` 添加一个条目。
...
- `configPatch`：可选的 `ancient-claw.json` 变更
...
当 provider 具有多种身份验证方法时，向导可以显式指向其中一种方法，也可以让 Ancient Claw 为每种方法自动生成选择项。

在插件注册时，Ancient Claw 会验证 provider 向导元数据：
...
- `ancient-claw models auth login --provider <id> [--method <id>]`
...
- `run` 会接收一个 `ProviderAuthContext`，其中包含 `prompter`、`runtime`、
  ...
  `allowSecretRefPrompt` 辅助工具/状态。新手引导/配置流程可以用它们来遵守 `--secret-input-mode`，或提供 env/file/exec secret-ref 采集，而 `ancient-claw models auth` 会保持更严格的提示表面。
...
- `wizard.setup` 会向 `ancient-claw onboard` 添加一个 provider 选项。
...
- 主包：`ancient-claw`（本仓库）
- 插件：位于 `@ancient-claw/*` 下的独立 npm 包（例如：`@ancient-claw/voice-call`）

发布契约：

- 插件 `package.json` 必须包含 `ancient-claw.extensions`，并指向一个或多个入口文件。
- 可选：`ancient-claw.setupEntry` 可指向一个轻量级的仅 setup 入口，用于已禁用或尚未配置完成的渠道 setup。
- 入口文件可以是 `.js` 或 `.ts`（jiti 会在运行时加载 TS）。
- `ancient-claw plugins install <npm-spec>` 会使用 `npm pack`，提取到 `~/.ancient-claw/extensions/<id>/`，并在配置中启用它。
...
- CLI：`ancient-claw voicecall start|status`
- 工具：`voice_call`
...
- 单独发布的插件应运行自己的 CI（lint/build/test），并验证 `ancient-claw.extensions` 指向构建后的入口点（`dist/index.js`）。
