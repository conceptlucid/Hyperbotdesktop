...
read_when:
  - Building or debugging native Ancient Claw plugins
...
This page covers the internal architecture of the Ancient Claw plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Ancient Claw. Every
native Ancient Claw plugin registers against one or more capability types:
...
Ancient Claw classifies every loaded plugin into a shape based on its actual
...
Use `ancient-claw plugins inspect <id>` to see a plugin's shape and capability
...
When you run `ancient-claw doctor` or `ancient-claw plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `ancient-claw status --all` and `ancient-claw plugins doctor`.

## Architecture overview

Ancient Claw's plugin system has four layers:

1. **Manifest + discovery**
   Ancient Claw finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `ancient-claw.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Ancient Claw plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Ancient Claw reads the registry to expose tools, channels, provider
...
That split lets Ancient Claw validate config, explain missing/disabled plugins, and
...
normal chat actions. Ancient Claw keeps one shared `message` tool in core, and
...
Ancient Claw treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Ancient Claw-facing
  surfaces
...
So if Ancient Claw adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Ancient Claw has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { Ancient ClawPluginDefinition } from "ancient-claw/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "ancient-claw/plugin-sdk";

const plugin: Ancient ClawPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Ancient Claw already treats image/audio/video understanding as one shared
...
If Ancient Claw adds a new domain later, such as video generation, use the same
...
`Ancient ClawPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Ancient Claw can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Ancient Claw knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `Ancient ClawPluginApi` or
  `api.runtime`
...
Native Ancient Claw plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the Ancient Claw process

Compatible bundles are safer by default because Ancient Claw currently treats them
...
name: `@ancient-claw/<id>` by default, or an approved typed suffix such as
...
Ancient Claw exports capabilities, not implementation convenience.
...
At startup, Ancient Claw does roughly this:
...
The manifest is the control-plane source of truth. Ancient Claw uses it to:
...
Ancient Claw keeps short in-process caches for:
...
Ancient Claw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Ancient Claw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Ancient Claw tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Ancient Claw's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Ancient Claw's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Ancient Claw adds those
...
- If Ancient Claw adds a new capability such as video generation later, define the
...
- Ancient Claw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Ancient Claw plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `ancient-claw/plugin-sdk` import when
authoring plugins:

- `ancient-claw/plugin-sdk/plugin-entry` for plugin registration primitives.
- `ancient-claw/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `ancient-claw/plugin-sdk/channel-setup`,
  `ancient-claw/plugin-sdk/channel-pairing`,
  `ancient-claw/plugin-sdk/channel-contract`,
  `ancient-claw/plugin-sdk/channel-feedback`,
  `ancient-claw/plugin-sdk/channel-inbound`,
  `ancient-claw/plugin-sdk/channel-lifecycle`,
  `ancient-claw/plugin-sdk/channel-reply-pipeline`,
  `ancient-claw/plugin-sdk/command-auth`,
  `ancient-claw/plugin-sdk/secret-input`, and
  `ancient-claw/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `ancient-claw/plugin-sdk/channel-config-helpers`,
  `ancient-claw/plugin-sdk/allow-from`,
  `ancient-claw/plugin-sdk/channel-config-schema`,
  `ancient-claw/plugin-sdk/channel-policy`,
  `ancient-claw/plugin-sdk/config-runtime`,
  `ancient-claw/plugin-sdk/infra-runtime`,
  `ancient-claw/plugin-sdk/agent-runtime`,
  `ancient-claw/plugin-sdk/lazy-runtime`,
  `ancient-claw/plugin-sdk/reply-history`,
  `ancient-claw/plugin-sdk/routing`,
  `ancient-claw/plugin-sdk/status-helpers`,
  `ancient-claw/plugin-sdk/runtime-store`, and
  `ancient-claw/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `ancient-claw/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `ancient-claw/plugin-sdk/*` subpaths. Ancient Claw core/test code may use the repo
...
- Avoid the root `ancient-claw/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `ancient-claw/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `ancient-claw/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `ancient-claw/plugin-sdk/<extension>`.
...
`ancient-claw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Ancient Claw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Ancient Claw's
...
- if both `catalog` and `discovery` are registered, Ancient Claw uses `catalog`
...
- Read-only command paths such as `ancient-claw status`, `ancient-claw status --all`,
  `ancient-claw channels status`, `ancient-claw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `ancient-claw.extensions`:
...
Security guardrail: every `ancient-claw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `ancient-claw plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `ancient-claw.setupEntry` can point at a lightweight setup-only module.
When Ancient Claw needs setup surfaces for a disabled channel plugin, or
...
Optional: `ancient-claw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Ancient Claw load the
...
Channel plugins can advertise setup/discovery metadata via `ancient-claw.channel` and
install hints via `ancient-claw.install`. This keeps the core catalog data-free.
...
Ancient Claw can also merge **external channel catalogs** (for example, an MPM
...
contain `{ "entries": [ { "name": "@scope/pkg", "ancient-claw": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "ancient-claw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `Ancient ClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Ancient Claw stays opinionated without becoming hardcoded to one
