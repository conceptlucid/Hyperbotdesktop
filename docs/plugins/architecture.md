...
read_when:
  - Building or debugging native HyperBot plugins
...
This page covers the internal architecture of the HyperBot plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside HyperBot. Every
native HyperBot plugin registers against one or more capability types:
...
HyperBot classifies every loaded plugin into a shape based on its actual
...
Use `hyperbot plugins inspect <id>` to see a plugin's shape and capability
...
When you run `hyperbot doctor` or `hyperbot plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `hyperbot status --all` and `hyperbot plugins doctor`.

## Architecture overview

HyperBot's plugin system has four layers:

1. **Manifest + discovery**
   HyperBot finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `hyperbot.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native HyperBot plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of HyperBot reads the registry to expose tools, channels, provider
...
That split lets HyperBot validate config, explain missing/disabled plugins, and
...
normal chat actions. HyperBot keeps one shared `message` tool in core, and
...
HyperBot treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's HyperBot-facing
  surfaces
...
So if HyperBot adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If HyperBot has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { HyperBotPluginDefinition } from "hyperbot/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "hyperbot/plugin-sdk";

const plugin: HyperBotPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
HyperBot already treats image/audio/video understanding as one shared
...
If HyperBot adds a new domain later, such as video generation, use the same
...
`HyperBotPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   HyperBot can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that HyperBot knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `HyperBotPluginApi` or
  `api.runtime`
...
Native HyperBot plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the HyperBot process

Compatible bundles are safer by default because HyperBot currently treats them
...
name: `@hyperbot/<id>` by default, or an approved typed suffix such as
...
HyperBot exports capabilities, not implementation convenience.
...
At startup, HyperBot does roughly this:
...
The manifest is the control-plane source of truth. HyperBot uses it to:
...
HyperBot keeps short in-process caches for:
...
HyperBot still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, HyperBot calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | HyperBot tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on HyperBot's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before HyperBot's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as HyperBot adds those
...
- If HyperBot adds a new capability such as video generation later, define the
...
- HyperBot only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "HyperBot plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `hyperbot/plugin-sdk` import when
authoring plugins:

- `hyperbot/plugin-sdk/plugin-entry` for plugin registration primitives.
- `hyperbot/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `hyperbot/plugin-sdk/channel-setup`,
  `hyperbot/plugin-sdk/channel-pairing`,
  `hyperbot/plugin-sdk/channel-contract`,
  `hyperbot/plugin-sdk/channel-feedback`,
  `hyperbot/plugin-sdk/channel-inbound`,
  `hyperbot/plugin-sdk/channel-lifecycle`,
  `hyperbot/plugin-sdk/channel-reply-pipeline`,
  `hyperbot/plugin-sdk/command-auth`,
  `hyperbot/plugin-sdk/secret-input`, and
  `hyperbot/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `hyperbot/plugin-sdk/channel-config-helpers`,
  `hyperbot/plugin-sdk/allow-from`,
  `hyperbot/plugin-sdk/channel-config-schema`,
  `hyperbot/plugin-sdk/channel-policy`,
  `hyperbot/plugin-sdk/config-runtime`,
  `hyperbot/plugin-sdk/infra-runtime`,
  `hyperbot/plugin-sdk/agent-runtime`,
  `hyperbot/plugin-sdk/lazy-runtime`,
  `hyperbot/plugin-sdk/reply-history`,
  `hyperbot/plugin-sdk/routing`,
  `hyperbot/plugin-sdk/status-helpers`,
  `hyperbot/plugin-sdk/runtime-store`, and
  `hyperbot/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `hyperbot/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `hyperbot/plugin-sdk/*` subpaths. HyperBot core/test code may use the repo
...
- Avoid the root `hyperbot/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `hyperbot/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `hyperbot/plugin-sdk/channel-actions`.
...
`hyperbot/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape HyperBot writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to HyperBot's
...
- if both `catalog` and `discovery` are registered, HyperBot uses `catalog`
...
- Read-only command paths such as `hyperbot status`, `hyperbot status --all`,
  `hyperbot channels status`, `hyperbot channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `hyperbot.extensions`:
...
Security guardrail: every `hyperbot.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `hyperbot plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `hyperbot.setupEntry` can point at a lightweight setup-only module.
When HyperBot needs setup surfaces for a disabled channel plugin, or
...
Optional: `hyperbot.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let HyperBot load the
...
Channel plugins can advertise setup/discovery metadata via `hyperbot.channel` and
install hints via `hyperbot.install`. This keeps the core catalog data-free.
...
HyperBot can also merge **external channel catalogs** (for example, an MPM
...
contain `{ "entries": [ { "name": "@scope/pkg", "hyperbot": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "hyperbot/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `HyperBotPluginApi` and/or `api.runtime` with the smallest useful
...
This is how HyperBot stays opinionated without becoming hardcoded to one
