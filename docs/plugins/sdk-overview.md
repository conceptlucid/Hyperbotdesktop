...
read_when:
  ...
  - You want a reference for all registration methods on HyperBotPluginApi
...
The `register(api)` callback receives an `HyperBotPluginApi` object with these
methods:
...
<Warning>
  Never import your own plugin through `hyperbot/plugin-sdk/<your-plugin>`
