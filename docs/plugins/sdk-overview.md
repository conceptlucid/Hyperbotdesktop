...
read_when:
  ...
  - You want a reference for all registration methods on Ancient ClawPluginApi
...
The `register(api)` callback receives an `Ancient ClawPluginApi` object with these
methods:
...
<Warning>
  Never import your own plugin through `ancient-claw/plugin-sdk/<your-plugin>`
