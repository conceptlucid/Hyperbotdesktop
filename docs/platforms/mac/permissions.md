...
- Same path: run the app from a fixed location (for Ancient Claw, `dist/Ancient Claw.app`).
...
```bash
sudo tccutil reset Accessibility ai.ancient-claw.mac
sudo tccutil reset ScreenCapture ai.ancient-claw.mac
...
Workaround: move files into the Ancient Claw workspace (`~/.ancient-claw/workspace`) if you want to avoid per-folder grants.
