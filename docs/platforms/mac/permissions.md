...
- Same path: run the app from a fixed location (for HyperBot, `dist/HyperBot.app`).
...
```bash
sudo tccutil reset Accessibility ai.hyperbot.mac
sudo tccutil reset ScreenCapture ai.hyperbot.mac
...
Workaround: move files into the HyperBot workspace (`~/.hyperbot/workspace`) if you want to avoid per-folder grants.
