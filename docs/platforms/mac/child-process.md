...
service via the external `hyperbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.hyperbot.gateway`
  (or `ai.hyperbot.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.hyperbot.*` is supported).
