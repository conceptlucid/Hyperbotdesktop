...
service via the external `ancient-claw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.ancient-claw.gateway`
  (or `ai.ancient-claw.<profile>` when using `--profile`/`OPENCLAW_PROFILE`; legacy `com.ancient-claw.*` is supported).
