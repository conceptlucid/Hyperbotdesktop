...
> HyperBot is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
HyperBot security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `hyperbot security audit`
...
```bash
hyperbot security audit
hyperbot security audit --deep
hyperbot security audit --fix
hyperbot security audit --json
```
...
HyperBot is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
HyperBot assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.hyperbot`, including `hyperbot.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- HyperBot can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want HyperBot, use one VPS/host per user.
...
HyperBot is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/hyperbot/hyperbot/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, HyperBot also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**:
  - `~/.hyperbot/credentials/<channel>-allowFrom.json` (default account)
  - `~/.hyperbot/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.hyperbot/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.hyperbot/secrets.json`
- **Legacy OAuth import**: `~/.hyperbot/credentials/oauth.json`
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full HyperBot state                                 | filesystem perms on `~/.hyperbot`                                                                    | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.hyperbot/hyperbot.json`                                                      | yes      |
...
| `tools.exec.auto_allow_skills_enabled`                        | warn          | Exec approvals trust skill bins implicitly                                           | `~/.hyperbot/exec-approvals.json`                                                                    | no       |
...
`hyperbot security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`hyperbot security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in HyperBot config
schema:
...
- HyperBot gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from HyperBot responses.
...
HyperBot stores session transcripts on disk under `~/.hyperbot/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.hyperbot` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If HyperBot cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
- If you don’t want remote execution, set security to **deny** and remove node pairing for that Mac.

## Dynamic skills (watcher / remote nodes)

HyperBot can refresh the skills list mid-session:
...
- If you install plugins (`hyperbot plugins install <package>`), treat it like running untrusted code:
  - The install path is `~/.hyperbot/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - HyperBot uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
hyperbot pairing list <channel>
hyperbot pairing approve <channel> <code>
```
...
By default, HyperBot routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
HyperBot has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.hyperbot/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.hyperbot or your logs.”

## Unsafe external content bypass flags

HyperBot includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.hyperbot/hyperbot.json`: `600` (user read/write only)
- `~/.hyperbot`: `700` (user only)

`hyperbot doctor` can warn and offer to tighten these permissions.
...
- Canvas host: `/__hyperbot__/canvas/` and `/__hyperbot__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run HyperBot with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_hyperbot-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `hyperbot doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), HyperBot
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. HyperBot verifies the identity by resolving the
...
- HyperBot will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.hyperbot/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

- `hyperbot.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `hyperbot status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@hyperbot", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.hyperbot`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.hyperbot/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `hyperbot` profile).
...
HyperBot’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default HyperBot limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `hyperbot gateway` process.
...
1. Check Gateway logs: `/tmp/hyperbot/hyperbot-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.hyperbot/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `hyperbot security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + HyperBot version
...
Found a vulnerability in HyperBot? Please report responsibly:

1. Email: [security@hyperbot.ai](mailto:security@hyperbot.ai)
