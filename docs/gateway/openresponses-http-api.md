...
Ancient Claw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`ancient-claw agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "ancient-claw"`, `model: "ancient-claw/default"`, `model: "ancient-claw/<agentId>"`, or `x-ancient-claw-agent-id`
- use `x-ancient-claw-model` when you want to override the selected agent's backend model
- use `x-ancient-claw-session-key` for explicit session routing
- use `x-ancient-claw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: Ancient Claw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
