...
HyperBot’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`hyperbot agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "hyperbot"`, `model: "hyperbot/default"`, `model: "hyperbot/<agentId>"`, or `x-hyperbot-agent-id`
- use `x-hyperbot-model` when you want to override the selected agent's backend model
- use `x-hyperbot-session-key` for explicit session routing
- use `x-hyperbot-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: HyperBot reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
