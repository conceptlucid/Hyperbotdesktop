---
summary: "How HyperBot sandboxing works: modes, scopes, workspace access, and images"
title: Sandboxing
...
HyperBot can run **tools inside sandbox backends** to reduce blast radius.
...
- Optional sandboxed browser (`agents.defaults.sandbox.browser`).
  ...
  - By default, sandbox browser containers use a dedicated Docker network (`hyperbot-sandbox-browser`) instead of the global `bridge` network.
  ...
  - noVNC observer access is password-protected by default; HyperBot emits a short-lived token URL that serves a local bootstrap page and opens noVNC with password in URL fragment (not query/header logs).
...
Use `backend: "ssh"` when you want HyperBot to sandbox `exec`, file tools, and media reads on
...
- HyperBot creates a per-scope remote root under `sandbox.ssh.workspaceRoot`.
- On first use after create or recreate, HyperBot seeds that remote workspace from the local workspace once.
...
- HyperBot does not sync remote changes back to the local workspace automatically.

Authentication material:

- `identityFile`, `certificateFile`, `knownHostsFile`: use existing local files and pass them through OpenSSH config.
- `identityData`, `certificateData`, `knownHostsData`: use inline strings or SecretRefs. HyperBot resolves them through the normal secrets runtime snapshot, writes them to temp files with `0600`, and deletes them when the SSH session ends.
...
- Host-local edits made outside HyperBot after the seed step are not visible remotely until you recreate the sandbox.
- `hyperbot sandbox recreate` deletes the per-scope remote root and seeds again from local on next use.
- Browser sandboxing is not supported on the SSH backend.
- `sandbox.docker.*` settings do not apply to the SSH backend.

### OpenShell backend

Use `backend: "openshell"` when you want HyperBot to sandbox tools in an
...
- `mirror` (default): local workspace stays canonical. HyperBot syncs local files into OpenShell before exec and syncs the remote workspace back after exec.
- `remote`: OpenShell workspace is canonical after the sandbox is created. HyperBot seeds the remote workspace once from the local workspace, then file tools and exec run directly against the remote sandbox without syncing changes back.

Remote transport details:

- HyperBot asks OpenShell for sandbox-specific SSH config via `openshell sandbox ssh-config <name>`.
...
- you edit files locally outside HyperBot and want those changes to show up in the sandbox automatically
...
- When the sandbox is first created, HyperBot seeds the remote workspace from the local workspace once.
- After that, `exec`, `read`, `write`, `edit`, and `apply_patch` operate directly against the remote OpenShell workspace.
- HyperBot does **not** sync remote changes back into the local workspace after exec.
...
- If you edit files on the host outside HyperBot after the seed step, the remote sandbox will **not** see those changes automatically.
...
- `hyperbot sandbox list` shows OpenShell runtimes as well as Docker runtimes
- `hyperbot sandbox recreate` deletes the current runtime and lets HyperBot recreate it on next use
...
- `"none"` (default): tools see a sandbox workspace under `~/.hyperbot/sandboxes`.
...
HyperBot mirrors eligible skills into the sandbox workspace (`.../skills`) so
...
- HyperBot blocks dangerous bind sources (for example: `docker.sock`, `/etc`, `/proc`, `/sys`, `/dev`, and parent mounts that would expose them).
...
Default Docker image: `hyperbot-sandbox:bookworm-slim`
...
`hyperbot-sandbox-common:bookworm-slim`.
...
- Use `hyperbot sandbox explain` to inspect effective sandbox mode, tool policy, and fix-it config keys.
