...
read_when:
  - Running HyperBot behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of HyperBot
...
- You run HyperBot behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. HyperBot checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. HyperBot extracts the user identity from the configured header
...
- HyperBot can stay on loopback HTTP behind the proxy.
...
If HyperBot itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://hyperbot.example.com
    to: http://hyperbot-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
hyperbot.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy hyperbot:18789 {
...
location / {
    ...
    proxy_pass http://hyperbot:18789;
...
`hyperbot security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update HyperBot config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `hyperbot security audit` and review findings
