...
read_when:
  - Running Ancient Claw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Ancient Claw
...
- You run Ancient Claw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Ancient Claw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Ancient Claw extracts the user identity from the configured header
...
- Ancient Claw can stay on loopback HTTP behind the proxy.
...
If Ancient Claw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://ancient-claw.example.com
    to: http://ancient-claw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
ancient-claw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy ancient-claw:18789 {
...
location / {
    ...
    proxy_pass http://ancient-claw:18789;
...
`ancient-claw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Ancient Claw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `ancient-claw security audit` and review findings
