...
Ancient Claw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Ancient Claw requires a shared password.
...
(`tailscale-user-login`) without supplying a token/password. Ancient Claw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
Ancient Claw only treats a request as Serve when it arrives from loopback with
...
- Set `gateway.tailscale.resetOnExit` if you want Ancient Claw to undo `tailscale serve`
