...
- During heartbeat runs, HyperBot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `hyperbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, HyperBot skips the heartbeat run entirely (no model call).
...
```bash
hyperbot system event --text "Check for urgent follow-ups" --mode now
```
