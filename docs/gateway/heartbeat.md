...
- During heartbeat runs, Ancient Claw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `ancient-claw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Ancient Claw skips the heartbeat run entirely (no model call).
...
```bash
ancient-claw system event --text "Check for urgent follow-ups" --mode now
```
