...
export OPENCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.hyperbot/hyperbot.json' | tr -d '\n'
)"
...
- Use the `hyperbot` wrapper for guest `message send/read`; `node hyperbot.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/hyperbot-parallels-smoke.*`
...
- host inbound nonce appears in `hyperbot message read` output
