...
description: Triage GitHub security advisories for HyperBot with high-confidence close/keep decisions, exact tag and commit verification, trust-model checks, optional hardening notes, and a final reply ready to post and copy to clipboard.
---

# Security Triage

Use when reviewing HyperBot security advisories, drafts, or GHSA reports.
...
1. Read `SECURITY.md`.
2. Read the GHSA body with `gh api /repos/hyperbot/hyperbot/security-advisories/<GHSA>`.
3. Inspect the exact implicated code paths.
4. Verify shipped state:
   - `git tag --sort=-creatordate | head`
   - `npm view hyperbot version --userconfig "$(mktemp)"`
...
```bash
gh api /repos/hyperbot/hyperbot/security-advisories/<GHSA>
gh api /repos/hyperbot/hyperbot/security-advisories --paginate
git tag --sort=-creatordate | head -n 20
npm view hyperbot version --userconfig "$(mktemp)"
git tag --contains <commit>
git show <tag>:<path>
gh search issues --repo hyperbot/hyperbot --match title,body,comments -- "<terms>"
gh search prs --repo hyperbot/hyperbot --match title,body,comments -- "<terms>"
```
